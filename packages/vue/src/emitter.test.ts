...
describe("VueEmitter", () => {
  ...
  it("emits accordion-specific state model", async () => {
    ...
    expect(file.content).toContain('type AccordionType = "single" | "multiple";');
    expect(file.content).toContain("const internalValue = ref<string | string[] | undefined>");
    expect(file.content).toContain("function toggleItem(itemValue: string): void");
